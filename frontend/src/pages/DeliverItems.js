...
export default function DeliverItems() {
  ...
  const fetchUndeliveredOrders = async () => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/seller/undelivered-orders`, {
        method: 'GET',
        headers: {
  ...
  const handleDeliveryConfirmation = async (orderId) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/seller/confirm-delivery`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  if (isLoading) {
    return (
      <>
        <Navbar title={process.env.REACT_APP_BRAND_NAME} />
