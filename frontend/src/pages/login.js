...
export default function Login() {
    ...
    useEffect(() => {
        const clearSession = async () => {
            try {
                await fetch(`${process.env.REACT_APP_API_URL}/api/clear-session`, {
                    method: 'POST',
                    credentials: 'include'
                });
            } catch (error) {
    ...
    const handleCASLogin = async () => {
        try {
            ...
            const casURL = process.env.REACT_APP_CAS_URL;
            const serviceURL = encodeURIComponent(`${process.env.REACT_APP_API_URL}/api/cas/validate`);
    ...
    const getErrorMessage = (name, value) => {
        switch (name) {
            case 'email':
                return value.trim()
                    ? `Email must be a valid IIIT domain (e.g., user@${process.env.REACT_APP_EMAIL_DOMAIN}).`
                    : 'Email is required.';
            case 'password':
                return value.trim()
            ...
            <form className={styles.formWrapper} onSubmit={handleSubmit}>
                <div>
                    ...
                    <input
                        ...
                        placeholder={`example@${process.env.REACT_APP_EMAIL_DOMAIN}`}
                        name="email"
